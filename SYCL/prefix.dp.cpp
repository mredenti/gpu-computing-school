             ...
             int totalSize, sycl::nd_item<1> item)
{
    ...
    int idx1 = idx0 + 1;
    temp[idx0] = (idx0 < totalSize) ? in[idx0] : 0.0;
    temp[idx1] = (idx1 < totalSize) ? in[idx1] : 0.0;
    ...
    for (int d = BLOCK >> 1; d > 0; d >>= 1) {
        ...
        if (thread_idx < d) {
            int ai = block_off + offset * (2 * thread_idx + 1) - 1;
            int bi = block_off + offset * (2 * thread_idx + 2) - 1;
            temp[bi] += temp[ai];
        }
    ...
    // clear last element
    if (thread_idx == (BLOCK / 2 - 1)) temp[block_off + BLOCK - 1] = 0.0;
...
int main() {
    ...
    auto devTmp = sycl::malloc_device<float>(BLOCK * nblocks, Q);
    ...
    std::cout << "KERNEL: Simple Prefix Sum SYCL" << std::endl;
